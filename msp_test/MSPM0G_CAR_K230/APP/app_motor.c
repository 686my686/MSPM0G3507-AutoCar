...
static float g_speed_filter[MAX_MOTOR] = {0};
...
void Motion_Stop(uint8_t brake)
{
    ...
    {
        g_speed_filter[i] = 0.0f;
        motor_data.speed_mm_s[i] = 0.0f;
    }
    Motor_Stop(brake);
}
...
void Motion_Get_Speed(car_data_t *car)
{
    ...
    /* 编码器底层已统一正方向，这里保留符号并进行一阶低通滤波。 */
    for (i = 0; i < MAX_MOTOR; i++)
    {
        g_speed_filter[i] = 0.35f * speed_mm[i] + 0.65f * g_speed_filter[i];
        motor_data.speed_mm_s[i] = g_speed_filter[i];
    }
...
void Motion_Handle(void)
{
    ...
    if (g_start_ctrl)
    {
        ...
        /* 赛题要求只能前进，正向目标下禁止负PWM触发反转 */
        if (motor_data.speed_set[0] > 0 && motor_data.speed_pwm[0] < 0)
        {
            motor_data.speed_pwm[0] = 0;
            pid_motor[0].pwm_output = 0;
        }
        if (motor_data.speed_set[1] > 0 && motor_data.speed_pwm[1] < 0)
        {
            motor_data.speed_pwm[1] = 0;
            pid_motor[1].pwm_output = 0;
        }
