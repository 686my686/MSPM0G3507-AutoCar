...
if(mode3_flag==0&&mode3_stop==0&&Line_flag==0&&yaw_flag==0)
	{
		delay_ms(2000);
		first_yaw =calibratedYaw; //获取此时imu数据用来走斜线 Obtain the IMU data at this moment in order to calculate the slanting angle.
	  object_yaw  = navigetion_0_360_limit(first_yaw-35);	//计算走斜线角度，如果斜线有偏差修改这个地方的参数 Calculate the angle of the diagonal line; if there is any deviation in the diagonal line, adjust the parameters in this area accordingly.
...
else if(mode3_flag==1&&mode3_stop==1&&Line_flag==0&&yaw_flag==0)
{
	...
	Motor_Back(100,100);
	delay_ms(1000);
	first_yaw_flag2=calibratedYaw;//获取此时imu角度，用来与imu进行判断从而控制小车离线停止 Obtain the current IMU angle, which will be used to make judgments in conjunction with the IMU data in order to control the vehicle to stop offline.
...
//		RGB_BEEP_flag=0;
	...
	if((odometry_sum>40)&&line_stop==0&&abs(first_yaw_flag2-calibratedYaw)>160&&abs(first_yaw_flag2-calibratedYaw)<180)//判断小车离线停止位置，如果停不准可以修改这个参数 Determine the offline stopping position of the cart; if the stopping position is inaccurate, this parameter can be adjusted accordingly.
	{
		yaw_flag=1;
		Motor_Stop(1) ;
	...
	Second_yaw=calibratedYaw;//获取此时imu数据用来走斜线 Obtain the IMU data at this moment in order to calculate the slanting angle.
		odometry_sum=0;
	mode3_stop= 0;
		
	object_yaw  = navigetion_0_360_limit(Second_yaw+37.4);//计算走斜线角度，如果斜线有偏差修改这个地方的参数 Calculate the angle of the diagonal line; if there is any deviation in the diagonal line, adjust the parameters in this area accordingly.
...
//		encoder_odometry_flag = 1;
		...
		Buzzer_close_state();
		Control_RGB_ALL(OFF);
//			Motor_Back(100,100);//如果你的小车走完斜线之后灰度总是超出线，可以修改这个让小车后退 If, after your car moves along the diagonal line, the gray level always exceeds the specified limit, you can modify the settings to make the car move back.
				delay_ms(1000);
